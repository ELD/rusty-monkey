...
pub fn lex_input(i: &str) -> Vec<Token> {
    let symbol_parsers = alt((static_token_lexer_generator("==", Token::Eq),
    ...
    let keyword_parsers = alt((static_token_lexer_generator("let", Token::Let),
...
fn static_token_lexer_generator<'a, E>(symbol: &'a str,
                                       token: Token)
...
fn ident_lexer<'a, E>() -> impl FnMut(&'a str) -> IResult<&'a str, Token, E>
    where E: ParseError<&'a str>
{
    map(preceded(whitespace,
                 preceded(peek(alt((alpha1, tag("_")))),
                          take_while1(|c: char| c.is_alphanumeric() || c == '_'))),
        |ident: &str| Token::Ident(ident.into()))
}

fn number_lexer<'a>() -> impl FnMut(&'a str) -> IResult<&'a str, Token> {
    map(preceded(whitespace, take_while1(|c: char| c.is_numeric())),
        |number: &str| Token::Int(number.parse::<i64>().unwrap()))
}

pub(crate) fn whitespace<'a, E: ParseError<&'a str>>(i: &'a str) -> IResult<&'a str, &'a str, E> {
...
mod test {
    ...
    fn test_tokens_simple() {
        ...
        let expected_tokens = vec![Token::Assign,
                                   Token::Plus,
                                   Token::LParen,
                                   Token::RParen,
                                   Token::LBrace,
...
10 != 9;
    ...
    fn ident_parser_test() {
        let inputs = vec![("five", Ok(("", Token::Ident("five".into())))),
                          ("+=", Err(Error(("+=", ErrorKind::Tag)))),
                          ("two", Ok(("", Token::Ident("two".into())))),
                          ("my_thing2", Ok(("", Token::Ident("my_thing2".into())))),
                          ("_private_ident", Ok(("", Token::Ident("_private_ident".into())))),
                          ("1nvalid_ident", Err(Error(("1nvalid_ident", ErrorKind::Tag)))),];
